...
describe('public site', () => {
  it('explains every part of the product and has one way in for customers', () => {
    ...
    for (const part of ['私信跟进', '内容运营', '车型库', '多账号管理', '客户案例', '安全可靠', '私有化部署', '常见问题', '预约演示']) {
  ...
  it('keeps its promises honest: real screens, a dated case, nothing claims what the product cannot do', () => {
    ...
    assert.match(html, /统计时间 2026 年 9 月 19 日至 21 日/, 'the case figures say when they were read');
    ...
    assert.match(html, /没有向门店开放读取私信的官方接口/, 'the DM-inbox limit is stated, not hidden');
    assert.match(html, /无法保证一定不被限流/);
    assert.ok(!DASHES.test(visible(html)), 'no em or en dashes');
    assert.ok(!/xiaohongshu-mcp|XHS_|REQUIRES_AUTH|UNAVAILABLE/i.test(html), 'no internals');
    const scripts = [...html.matchAll(/<script\b[^>]*>/g)].map((m) => m[0]);
    assert.deepEqual(scripts.map((t) => /src="\/assets\/site\.js\?v=[a-f0-9]+" defer/.test(t)), [true], 'one same-origin script and no inline code');
  ...
  it('echoes the visitor input after an error and confirms a stored request', () => {
    ...
    assert.match(landingPage({ sent: true }), /提交成功/);
  });
